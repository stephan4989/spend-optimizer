...
def extract_model_fit(
    ...
    """
    Compute actual vs model-predicted time series from posterior samples.

    Predicted = sum of all channel Hill-Adstock contributions at each time step,
    averaged over posterior samples with 10/90 percentile CI.
    This avoids sample_posterior_predictive() which doubles peak memory usage.
    ...
    # Sum contributions across all channels for each sample and time step
    total_contrib = np.zeros((n_samples, n_times))
    ...
    for i, ch in enumerate(fit_result.channel_names):
        ...
        total_contrib += beta[:, np.newaxis] * hill * fit_result.kpi_scale

    return {
        "dates": dates,
        "actual": actual,
        "predicted_mean": total_contrib.mean(axis=0).tolist(),
        "predicted_lower": np.percentile(total_contrib, 10, axis=0).tolist(),
        "predicted_upper": np.percentile(total_contrib, 90, axis=0).tolist(),
    }
